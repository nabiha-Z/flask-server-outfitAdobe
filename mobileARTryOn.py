...
def mobileTryOn(uploaded_file,dress):
    stopAR = False
    Button(root, text="Clickable", font=("times new roman", 20,"bold"),bg="black", fg="white").pack(expand=True, pady=20)
    mp_pose = mp.solutions.pose
    pose = mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) 
    startdistance = None
    neck_point_xaxis = 500
    ...
    while (int(time.time() - start_time) < capture_duration):
        ...
        if stopAR == False:
            ...
            cloth = cv2.resize(cloth, (345,555))
            ...
            try:
                if(bodylmlist[12] and bodylmlist[11]):
                    ...
                    if(length<20):
                        ...
                        cloth = cv2.imread('dressimg.png', cv2.IMREAD_UNCHANGED)
                        cloth = cv2.resize(cloth, (290,430))
                        LXP1=(bodylmlist[11][1])-0.15
                        LYP2=(bodylmlist[11][2])
                        LS1=LXP1,LYP2
                        LXP3=(bodylmlist[11][1])+0.01
                        ...
                        cloth = cv2.imread('dresses/dressimg1.png', cv2.IMREAD_UNCHANGED)
                        cloth = cv2.resize(cloth, (300,470))
                        length_of_shoulders = abs(bodylmlist[12][1]-bodylmlist[11][1])
                        startdistance = length_of_shoulders
                        length_of_shoulders = abs(bodylmlist[12][1]-bodylmlist[11][1])
                        scaling = int((length_of_shoulders-startdistance)/2)
                        length = bodylmlist[26][2]-115
...
mobileTryOn('TestingVideo7.mp4','dressimg1.png')