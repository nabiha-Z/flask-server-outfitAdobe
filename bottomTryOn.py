...
while (cap.isOpened()):
    ...
    if stopAR == False:
        ...
        cloth = cv2.imread("jeans4.png", cv2.IMREAD_UNCHANGED)
        cloth = cv2.resize(cloth, (250,300))
        ...
        try:
            if(bodylmlist[24] and bodylmlist[23]):
                if startdistance is None:
                    length_of_shoulders = abs(bodylmlist[24][1]-bodylmlist[23][1])
                    startdistance = length_of_shoulders
                    length_of_shoulders = abs(bodylmlist[24][1]-bodylmlist[23][1])
                    scaling = int((length_of_shoulders-startdistance)/2)
                    full = bodylmlist[28][2]-30
                    neck_point_xaxis = int(abs(bodylmlist[24][1]+bodylmlist[23][1])/2)
                    neck_point_yaxis = int(abs(bodylmlist[24][2]+full)/2)
                else:
                    startdistance = None
        except:
            pass
